package com.sulake.core.window.utils
{
   ...
   public class FocusManager extends ChildEntityArrayReader implements IWindowFocusManagerService
   {
      ...
      public function FocusManager(param1:DisplayObject)
      {
         ...
         var_207.addEventListener(Event.ACTIVATE,onActivateEvent);
         var_207.addEventListener(FocusEvent.FOCUS_OUT,onFocusEvent);
         var_207.addEventListener(FocusEvent.KEY_FOCUS_CHANGE,onFocusEvent);
         var_207.addEventListener(FocusEvent.MOUSE_FOCUS_CHANGE,onFocusEvent);
         super();
      }
      ...
      public function dispose() : void
      {
         if(!_disposed)
         {
            var_207.removeEventListener(Event.ACTIVATE,onActivateEvent);
            var_207.removeEventListener(FocusEvent.FOCUS_OUT,onFocusEvent);
            var_207.removeEventListener(FocusEvent.KEY_FOCUS_CHANGE,onFocusEvent);
            var_207.removeEventListener(FocusEvent.MOUSE_FOCUS_CHANGE,onFocusEvent);
            var_207 = null;
            _disposed = true;
            var_32 = null;
         }
