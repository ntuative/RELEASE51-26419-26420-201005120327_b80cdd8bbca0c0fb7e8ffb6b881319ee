package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget, IPetDataListener, IAvatarImageListener
   {
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(var_430 >= var_314.length)
         {
            return "";
         }
         var petColor:IPetColor = var_314[var_430] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:IBreed = var_617[var_1558] as IBreed;
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         var_430 = param1.index;
         if(var_430 < 0 || var_430 > var_314.length)
         {
            var_430 = 0;
         }
         updateImage();
      ...
      override public function init() : void
      {
         ...
         var_900 = getPetTypeIndexFromProduct(_loc5_.localizationId);
         var_314 = [];
         var_617 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc11_ = _loc10_.avatarRenderManager.petDataManager.getPetData(var_900,this);
               if(_loc11_)
               {
                  var_617 = _loc11_.breeds;
         ...
         var _loc6_:int = 0;
         while(_loc6_ < var_617.length)
         {
            _loc12_ = var_617[_loc6_] as IBreed;
            _loc3_.push(_loc4_.getKey(getRaceLocalizationKey(var_900,_loc12_.id),getRaceLocalizationKey(var_900,_loc12_.id)));
            _loc6_++;
         }
         var_569.add(var_900,_loc5_);
         ...
         events.addEventListener(WidgetEvent.const_860,onApproveNameResult);
         _loc5_ = var_569.getWithIndex(0);
         if(_loc5_ != null)
         {
      ...
      private function onApproveNameResult(param1:CatalogWidgetApproveNameResultEvent) : void
      {
         ...
         (page.viewer.catalog as HabboCatalog).showPurchaseConfirmation(var_569.getWithIndex(0),page,p);
      }
      ...
      {
         var _loc7_:* = null;
         var _loc1_:Offer = var_569.getWithIndex(0);
         if(_loc1_ == null)
         {
            return;
         }
         ...
         if(var_430 >= 0 && var_430 < var_314.length)
         {
            _loc7_ = var_314[var_430] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:IBreed = var_617[var_1558] as IBreed;
         if(!_loc3_)
         {
            return;
         }
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImage(var_900,_loc3_.id,_loc2_,AvatarScaleType.const_56,this);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_321,AvatarAction.const_629);
         _loc4_.endActionAppends();
         _loc4_.setDirection(AvatarSetType.const_41,2);
         setPreviewImage(_loc4_.getCroppedImage(AvatarSetType.const_41),true);
         _loc4_.dispose();
