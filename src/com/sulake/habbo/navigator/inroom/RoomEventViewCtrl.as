package com.sulake.habbo.navigator.inroom
{
   ...
   public class RoomEventViewCtrl
   {
      ...
      private var var_945:IDropMenuWindow;
      ...
      private function onEditButtonClick(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         var _loc5_:String = var_771.getText();
         var _loc6_:Array = new Array();
         addTag(_loc6_,var_946.getText());
      ...
      private function prepareWindow(param1:IWindowContainer) : void
      {
         if(_window == null)
         {
            ...
            var_1604 = IWindowContainer(find("event_type_disabled_cont"));
            var_2232 = ITextWindow(find("event_type_disabled_text"));
            var_1605 = ITextWindow(find("create_caption"));
            var_1603 = ITextWindow(find("edit_caption"));
            var_1381 = IButtonWindow(find("create_button"));
            var_1382 = IButtonWindow(find("edit_button"));
            var_2233 = IButtonWindow(find("cancel_button"));
            var_1383 = IButtonWindow(find("end_button"));
            var_945 = IDropMenuWindow(find("event_type"));
            prepareEventTypes();
            Util.setProcDirectly(var_1381,onCreateButtonClick);
            Util.setProcDirectly(var_1382,onEditButtonClick);
            Util.setProcDirectly(var_1383,onEndButtonClick);
            Util.setProcDirectly(var_2233,onCancelButtonClick);
            _eventNameInput = new TextFieldManager(_navigator,getInput("event_name"),25);
            var_771 = new TextFieldManager(_navigator,getInput("event_desc"),100);
            var_946 = new TextFieldManager(_navigator,getInput("event_tag_1"),25);
      ...
      private function isMandatoryFieldsFilled() : Boolean
      {
         ...
         if(!var_771.checkMandatory(_navigator.getText("navigator.eventsettings.descerr")))
         {
            return false;
         }
         return true;
      ...
      {
         var_945.visible = true;
         var_1604.visible = false;
         var_1381.visible = true;
         var_1382.visible = false;
         var_1383.visible = false;
         var_771.goBackToInitialState();
         _eventNameInput.goBackToInitialState();
         var_946.goBackToInitialState();
         var_944.goBackToInitialState();
         var_945.selection = 0;
         var_1605.visible = true;
