package com.sulake.habbo.ui
{
   ...
   public class RoomQueueWidgetHandler implements IRoomWidgetHandler
   {
      ...
      public function processEvent(param1:Event) : void
      {
         ...
         switch(param1.type)
         {
            case RoomSessionQueueEvent.const_381:
               ...
               {
                  case RoomSessionQueueEvent.const_887:
                     _loc3_ = "null";
                     break;
                  case RoomSessionQueueEvent.const_827:
                     _loc3_ = "null";
               }
               if(_loc3_ == null)
               {
               ...
               if(_loc7_.length > 1)
               {
                  if(_loc6_ && _loc2_.queueTypes.indexOf(RoomSessionQueueEvent.const_844) != -1)
                  {
                     _loc8_ = _loc2_.getQueueSize(RoomSessionQueueEvent.const_844) + 1;
                     _loc9_ = true;
                  }
                  else
                  {
      ...
      {
         var _loc1_:* = [];
         _loc1_.push(RoomWidgetRoomQueueMessage.const_649);
         _loc1_.push(RoomWidgetRoomQueueMessage.CHANGE_TO_SPECTATOR_QUEUE);
         _loc1_.push(RoomWidgetRoomQueueMessage.const_727);
         _loc1_.push(RoomWidgetRoomQueueMessage.const_752);
         return _loc1_;
      }
      ...
      public function processWidgetMessage(param1:RoomWidgetMessage) : RoomWidgetUpdateEvent
      {
         ...
         switch(param1.type)
         {
            case RoomWidgetRoomQueueMessage.const_649:
               _container.roomSession.quit();
               break;
            case RoomWidgetRoomQueueMessage.CHANGE_TO_SPECTATOR_QUEUE:
               _container.roomSession.changeQueue(RoomSessionQueueEvent.const_827);
               break;
            case RoomWidgetRoomQueueMessage.const_727:
               _container.roomSession.changeQueue(RoomSessionQueueEvent.const_887);
               break;
            case RoomWidgetRoomQueueMessage.const_752:
               if(false)
               {
                  _container.catalog.openCatalogPage(CatalogPageName.const_223,true);
               }
         }
         return null;
      }
      ...
      {
         return [RoomSessionQueueEvent.const_381];
      }
